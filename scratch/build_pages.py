    ...
    <div class="proposal-widget-body">
      ...
      <div class="widget-expanded-content">
        ...
        <ul class="widget-highlights" style="margin: 10px 0; padding-left: 0; list-style: none; display: flex; flex-direction: column; gap: 8px;">
          <li style="position: relative; padding-left: 18px; font-size: 0.775rem; line-height: 1.4; color: var(--color-text-main);"><strong style="color: var(--color-primary-navy);">Modern Multi-Page Layout:</strong> A structured 5-page setup (Home, Programs, Team, Rates, Contact) to make extensive services easy to digest and navigate.</li>
          <li style="position: relative; padding-left: 18px; font-size: 0.775rem; line-height: 1.4; color: var(--color-text-main);"><strong style="color: var(--color-primary-navy);">Inclusive Design &amp; Accessibility:</strong> Tailored high-contrast, dyslexia-friendly colors, custom scrollbars, and clear landmark structures for cognitive ease.</li>
          <li style="position: relative; padding-left: 18px; font-size: 0.775rem; line-height: 1.4; color: var(--color-text-main);"><strong style="color: var(--color-primary-navy);">Dynamic Anti-Scroll UX:</strong> Collapsible team bios (natural sentence previews with "Read More" expanders) that prevent infinite scrolling.</li>
          <li style="position: relative; padding-left: 18px; font-size: 0.775rem; line-height: 1.4; color: var(--color-text-main);"><strong style="color: var(--color-primary-navy);">Interactive Premium Features:</strong> Auto-rotating testimonials slider, filterable program category tabs, and smooth animated FAQ accordions.</li>
          <li style="position: relative; padding-left: 18px; font-size: 0.775rem; line-height: 1.4; color: var(--color-text-main);"><strong style="color: var(--color-primary-navy);">Live Utility Badges:</strong> Pulsating real-time "Office Hours" status indicator and persistent, direct client portal navigation links.</li>
          <li style="position: relative; padding-left: 18px; font-size: 0.775rem; line-height: 1.4; color: var(--color-text-main);"><strong style="color: var(--color-primary-navy);">Floating Socials &amp; Navigation:</strong> Glassmorphic right-hand charms bar and scroll-to-top button with smart responsive auto-hide queries.</li>
        </ul>
...
def build_team():
    ...
    counseling_team_raw = [m for m in staff_list if m["category"] == "counseling" and m["key"] not in ["mike-edwards"]]
    # Reorder counseling team to place Marty first, then Devin, and then the rest of the team to match leadership hierarchy and remain consistent
    counseling_team = []
    # Marty first
    for m in counseling_team_raw:
        if m["key"] == "marty-edwards":
            counseling_team.append(m)
            break
    # Devin second
    for m in counseling_team_raw:
        if m["key"] == "devin-mcclure":
            counseling_team.append(m)
            break
    # The rest alphabetically (as they are ordered in staff.json)
    for m in counseling_team_raw:
        if m["key"] not in ["marty-edwards", "devin-mcclure"]:
            counseling_team.append(m)
